...
- 2026-05-05: Implemented the Adaptive TACC selector as an amortized-complexity controller. The selector validates cheap policies first, considers DQN only when cheap policies are close or unstable, scores candidates with objective, variance, replication pressure, compute cost, and DQN instability credit, and records gate/score decisions in `results/final/run_metadata.json`.
...
- 2026-05-05: Corrected Adaptive TACC to match the sequential online cache-update goal. Relocation is now charged against the previously deployed Adaptive TACC placement, and DQN is implemented as transition-aware online refinement from the current cache state rather than as a fixed alternative placement. The final rerun selects Online DQN at all four perturbation windows, with Adaptive TACC objectives 2.209, 3.398, 3.284, and 3.555 for \(p=0.00,0.05,0.10,0.15\), respectively.
