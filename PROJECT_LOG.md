...
- 2026-05-05: Initial cost-aware selector update selected topology-aware greedy at perturbation 0.00, diversified popularity at 0.05 and 0.10, and hybrid DQN at 0.15. This result was later superseded when relocation was corrected to the sequential online cache-update objective.
...
- 2026-05-05: Added an extended methodological-clarification appendix so the paper better explains the reasoning behind the validation gate, perturbation semantics, sanity checks, and reproducibility protocol.
...
- 2026-05-05: Reworked the final paper away from project-report structure. Removed the compiled implementation section, removed the appendix-style clarification section, removed the introduction roadmap paragraph, expanded Introduction/Background/Related Work/System Model/Methodology/Evaluation/Discussion, and moved adaptation and perturbation explanations into research-method sections. Verified the compiled final paper uses 39 active citations and has no uncited 2024+ bibliography entries.
- 2026-05-05: Implemented the validation gate as an amortized-complexity controller. The gate validates cheap policies first, considers DQN only when cheap policies are close or unstable, scores candidates with objective, variance, replication pressure, compute cost, and DQN instability credit, and records gate/score decisions in `results/final/run_metadata.json`.
- 2026-05-05: Reviewed `update.md` as a paper-quality checklist. Expanded the final paper's methodology contributions, clarified why the perturbation sweep uses \(p \leq 0.15\), explained what should happen for larger perturbations, added a strategy-by-regime results table, clarified replication/redundancy/relocation measurement in both methods and results, improved the dataset caption, and cleaned questionable BibTeX entries against publisher/DOI metadata.
- 2026-05-05: Corrected the online controller to match the sequential cache-update goal. Relocation is now charged against the previously deployed online placement, and DQN is implemented as transition-aware online refinement from the current cache state rather than as a fixed alternative placement. The final rerun selects Online DQN at all six perturbation windows, with validation-gated objectives 2.209, 3.398, 3.284, 3.555, 3.307, and 4.415 for \(p=0.00,0.05,0.10,0.15,0.20,0.25\), respectively.
- 2026-05-05: Reframed the paper so the validation gate is presented as a safeguard and diagnostic check, not as a standalone adaptive-policy contribution. The extended perturbation sweep through \(p=0.25\) shows no meaningful switch away from transition-aware Online DQN; keep-current is competitive at \(p=0.25\), which is noted as a calibration caution.
