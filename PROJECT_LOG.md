...
- 2026-05-05: An earlier cost-aware selection experiment was superseded when relocation was corrected to the sequential online cache-update objective.
...
- 2026-05-05: Added extended methodological clarification so the paper better explains perturbation semantics, sanity checks, and reproducibility protocol.
...
- 2026-05-05: Implemented and then removed a cost-aware selection layer after the extended sweep showed that it duplicated Online DQN in the reported results. The cleaned paper now focuses on transition-aware Online DQN and keeps no-update only as a baseline.
...
- 2026-05-05: Corrected the online controller to match the sequential cache-update goal. Relocation is now charged against the previously deployed online placement, and DQN is implemented as transition-aware online refinement from the current cache state rather than as a fixed alternative placement. The extended perturbation sweep through \(p=0.25\) shows that Online DQN is the main learned online approach, while Keep Current is competitive at \(p=0.25\), which is noted as a calibration caution.
- 2026-05-05: Removed the prior selection-layer framing from the paper and generated assets. The final narrative is now cleaner: topology-aware baselines establish structure, fixed Greedy+DQN is an ablation, Keep Current is the no-update baseline, and Online DQN is the transition-aware online method. Tables now report objective, access, hit ratio, replication, redundancy, and relocation together.
