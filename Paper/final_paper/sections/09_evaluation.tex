...
At \(p=0.05\), \(p=0.10\), \(p=0.15\), \(p=0.20\), and \(p=0.25\), Online DQN obtains objectives 3.398, 3.284, 3.555, 3.307, and 4.415. The high-perturbation result is especially important: fixed Greedy + DQN has objective 6.280 at \(p=0.25\), while Online DQN reaches 4.415 because it refines the placement already deployed by the controller instead of asking the network to switch to a separately trained fixed placement. However, the no-update baseline has observed objective 4.342 at \(p=0.25\). Since lower objective is better, Keep Current is slightly better than Online DQN in this window, which shows that future controllers should include statistical confidence before rewriting under severe perturbation.
