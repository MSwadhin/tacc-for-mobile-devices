...
The intended real-world source is the CRAWDAD Dartmouth campus WLAN dataset. The current official Dartmouth page describes the 2009 release as containing more than five years of syslog, SNMP, and tcpdump traces from a campus deployment with more than 450 APs and several thousand users \cite{kotz_dartmouth_campus_2009}. Dartmouth WLAN releases and mobility studies have been widely used for AP-level WLAN mobility modeling \cite{c71593-22,kim2005mobility_wifi_ap}.
...
Records are sorted by client and timestamp. \texttt{OFF} intervals are removed before graph construction because they represent absence from the WLAN. Consecutive AP changes define handoffs. For each client sequence \(i_1,i_2,\ldots,i_T\), each transition \(i_t\rightarrow i_{t+1}\) with \(i_t\ne i_{t+1}\) increments handoff count \(H_{i_t,i_{t+1}}\). The evaluation uses the most active APs and keeps the largest connected component.

\begin{table}[t]
\centering
\caption{Dataset and graph statistics used in the evaluation.}
...
\begin{table}[t]
\centering
\caption{Dataset artifacts used by the evaluation pipeline.}
