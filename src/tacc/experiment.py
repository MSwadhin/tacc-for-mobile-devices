...
class ExperimentConfig:
    ...
    dqn_episodes: int = 85
    dqn_steps: int = 16
    screening_variance_weight: float = 0.25
    screening_replication_weight: float = 20.0
    screening_compute_weight: float = 0.03
...
def _screening_score(policy: str, validation_frame: pd.DataFrame, rate: float, config: ExperimentConfig) -> float:
    volatility_penalty = (
        config.screening_replication_weight * (rate**2) * validation_frame["replication_cost"].mean()
    )
    compute_penalty = config.screening_compute_weight * POLICY_COMPLEXITY.get(policy, 0.50)
    return float(
        validation_frame["objective"].mean()
        + config.screening_variance_weight * validation_frame["objective"].std(ddof=1)
        + volatility_penalty
        + compute_penalty
    )
...
def run_experiment(config: ExperimentConfig) -> dict[str, Path]:
    ...
    dqn_screening: dict[str, dict[str, float | bool | str]] = {}
    for rate in config.perturbation_rates:
        ...
        for policy in ["diversified_popularity", "topology_greedy"]:
            ...
            base_scores[policy] = _screening_score(policy, frame, rate, config)
    ...
    previous_online_policy = "bootstrap_topology_greedy"
    ...
    for rate in config.perturbation_rates:
        ...
        for sample in range(config.eval_samples):
            ...
            detail_rows.append(
                {
                    "policy": "keep_current",
                    "previous_policy": previous_online_policy,
        ...
        online_reference = online_dqn.copy()
        previous_online_policy = "online_dqn"
    ...
    graph_stats = {
        ...
        "dqn_screening": dqn_screening,
        "online_relocation_reference": "Online DQN is evaluated against the previously deployed online placement; the online horizon is bootstrapped from topology-aware greedy.",
